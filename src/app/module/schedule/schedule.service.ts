...
const createScheduleIntoDB = async (
    ...
    const totalSlots = Math.floor(
        durationInMinutes / MINUTES_ALLOCATED_PER_SLOT,
    );
...
const getMySchedulesIntoDB = async (query: IQuery, user: RequestUser) => {
...
const getAllSchedulesIntoDB = async (query: IQuery) => {
    const limit = query.limit ? Number(query.limit) : 10;
    const page = query.page ? Number(query.page) : 1;
    const skip = (page - 1) * limit;
    const sortBy = query.sortBy ? query.sortBy : 'createdAt';
...
const getScheduleByIdIntoDB = async (scheduleId: string) => {
...
const publishScheduleIntoDB = async (scheduleId: string, user: RequestUser) => {
    const doctor = await prisma.doctor.findUnique({
        where: { userId: user.id },
    });
...
const deleteScheduleIntoDB = async (scheduleId: string, user: RequestUser) => {
    const doctor = await prisma.doctor.findUnique({
        where: { userId: user.id },
    });
...
const getTodaysSchedulesIntoDB = async (query: IQuery) => {
    if (!query.doctorId) {
        throw new AppError(
...
export const ScheduleServices = {
    ...
    getMySchedulesIntoDB,
    getAllSchedulesIntoDB,
    getScheduleByIdIntoDB,
    updateScheduleIntoDB,
    publishScheduleIntoDB,
    deleteScheduleIntoDB,
    getTodaysSchedulesIntoDB,
};
